module region_mod_timed
   ...
   public initialize_region_mod, sample_marginalised_regions, output_maps, get_single_pixel_chisq_singlepar, get_region_like, get_region_like_singlepar, get_scrambled_pixlist, times
      ...
      do j = 0, npix-1
         do i = 1, num_components
            ...
            if (nneighbour_region == 0) then
               ...
               call cpu_time(dt(1))
               times(2) = times(2) + dt(1) - dt(2)
            else
               if (rand_uni(rng_handle) < 0.5) then
                  call cpu_time(dt(1))
                  call form_new_region(pix, i)
                  times(3) = times(3) + dt(1) - dt(2)
               else
...
!            print *, 'reject'
   ...
   function get_region_chisq_singlepar(par, region_state)
      ...
      !$OMP CRITICAL
      get_region_chisq_singlepar = get_region_chisq_singlepar + & 
            & get_region_chisq_singlepar_private
      !$OMP END CRITICAL

      !$OMP END PARALLEL
      call cpu_time(dt(2))
      times(19) = times(19) + dt(2) - dt(1)
   ...
   subroutine get_maxpoint_and_qrombint(mode, state, maxpoint, intres)
      ...
      if (mode == 1) then
         ...
         call cpu_time(dt(2))
         times(21) = times(21) + dt(2) - dt(1)
         do while (lnL < 1.d-5)
            delta = 0.5d0 * delta
            lnL = get_single_pixel_like_singlepar(maxpoint - delta, state, lnL0)
         end do
         call cpu_time(dt(1))
         times(22) = times(22) + dt(1) - dt(2)
         do while (lnL > 1.d-5)
            delta = 2.d0 * delta
            lnL = get_single_pixel_like_singlepar(maxpoint - delta, state, lnL0)
         end do
         call cpu_time(dt(2))
         times(23) = times(23) + dt(2) - dt(1)
         ...
         lnL = get_single_pixel_like_singlepar(maxpoint + delta, state, lnL0)
         call cpu_time(dt(2))
         times(24) = times(24) + dt(2) - dt(1)
         do while (lnL < 1.d-5)
            delta = 0.5d0 * delta
            lnL = get_single_pixel_like_singlepar(maxpoint + delta, state, lnL0)
         end do
         call cpu_time(dt(1))
         times(24) = times(24) + dt(1) - dt(2)
         do while (lnL > 1.d-5)
            delta = 2.d0 * delta
            lnL = get_single_pixel_like_singlepar(maxpoint + delta, state, lnL0)
         end do
         ...
         call cpu_time(dt(2))
         times(27) = times(27) + dt(2) - dt(1)
         do while (lnL < 1.d-5)
            delta = 0.5d0 * delta
            lnL = get_region_like_singlepar(maxpoint - delta, state, lnL0)
         end do
         call cpu_time(dt(1))
         times(28) = times(28) + dt(1) - dt(2)
         do while (lnL > 1.d-5)
            delta = 2.d0 * delta
            lnL = get_region_like_singlepar(maxpoint - delta, state, lnL0)
         end do
         call cpu_time(dt(2))
         times(29) = times(29) + dt(2) - dt(1)
         int_low = maxpoint - delta
         if (int_low < prior_low(component)) then
            int_low = prior_low(component)
         end if
         ...
         lnL = get_region_like_singlepar(maxpoint + delta, state, lnL0)
         call cpu_time(dt(2))
         times(30) = times(30) + dt(2) - dt(1)
         do while (lnL < 1.d-5)
            delta = 0.5d0 * delta
            lnL = get_region_like_singlepar(maxpoint + delta, state, lnL0)
         end do
         call cpu_time(dt(1))
         times(31) = times(31) + dt(1) - dt(2)
         do while (lnL > 1.d-5)
            delta = 2.d0 * delta
            lnL = get_region_like_singlepar(maxpoint + delta, state, lnL0)
         end do
         call cpu_time(dt(2))
         times(32) = times(32) + dt(2) - dt(1)
         int_high = maxpoint + delta
         if (int_high > prior_high(component)) then
            int_high = prior_high(component)
